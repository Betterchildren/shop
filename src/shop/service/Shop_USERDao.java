...
String sql = "insert into Shop_USER values(?, ?, ?, ?, DATE_FORMAT(?, '%Y-%m-%d'), ?, ?, ?, ?, ?)";
	...
	public static int update(Shop_USER u) {
		String sql = "update Shop_USER set USER_NAME=?, USER_PASSWORD=?,USER_SEX=?,USER_BIRTHDAY=DATE_FORMAT(?, '%y-%m-%d'),USER_IDENITY_CODE=?,USER_EMAIL=?,USER_MOBILE=?,USER_ADDRESS=?,USER_STATUS=? where USER_ID = ?";
	...
	public static Shop_USER selectByID(String id) {
		...
		try {
			String sql = "select m.*, DATE_FORMAT(m.user_birthday, '%Y-%m-%d')birthday  from Shop_USER m where USER_ID=?";
	...
	Shop_USER u = null;
		...
		try {
			String sql = "select m.*, DATE_FORMAT(m.user_birthday, '%Y-%m-%d')birthday  from Shop_USER m where USER_ID=? and USER_PASSWORD=?";
